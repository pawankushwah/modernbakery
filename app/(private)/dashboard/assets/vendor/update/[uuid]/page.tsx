...
import { updateVendor, vendorByUUID } from "@/app/services/assetsApi";
...
export default function UpdateVendorPage() {
  ...
  useEffect(() => {
    const fetchVendor = async () => {
      ...
      setLoading(false);
      if(res.error) {
          showSnackbar(res.data.message || "Unable to fetch the vendor", "error");
          throw new Error("unable fetch vendor");
      } else {
        setVendor(res.data);
      }
    };
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, resetForm }) => {
        setLoading(true);
        const res = await updateVendor(uuid, {
        ...
        if(res.error) {
          showSnackbar(res.data.message || "Failed to updated Vendor", "error");
          throw new Error("Unable to updated Vendor");
        } else {
          showSnackbar( res.message || "Vendor Updated successfully", "success");
          resetForm();
