...
export default function AddEditAgentCustomer() {
  ...
  const handleChange = async (
    ...
    // Validate only this field using Yup
    try {
      await AgentCustomerSchema.validateAt(name, { ...form, [name]: value });
      // If valid, remove error for this field
      setErrors((prev) => {
        const newErrors = { ...prev };
        delete newErrors[name as keyof AgentCustomerFormValues];
        return newErrors;
      });
    } catch (err) {
      if (err instanceof Yup.ValidationError) {
        setErrors((prev) => ({
          ...prev,
          [name as keyof AgentCustomerFormValues]: err.message,
        }));
      }
    }
  };
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <InputFields
                    ...
                    name="customer_type"
                    value={form.customer_type?.toString() ?? ""}
                    onChange={handleChange}
                ...
                <div>
                  <InputFields
                    ...
                    label="Is Whatsapp"
                    name="is_whatsapp"
                    value={form.is_whatsapp?.toString() ?? ""}
                    type="radio"
                    onChange={handleChange}
                    error={touched.is_whatsapp && errors.is_whatsapp}
                    options={[
                      { value: "1", label: "Yes" },
                      { value: "0", label: "No" },
                    ]}
                ...
                {form.is_whatsapp?.toString() === "1" && (
                  <div>
                    <InputFields
                      ...
                      onChange={handleChange}
                      error={touched.whatsapp_no && errors.whatsapp_no}
                    />
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <InputFields
                    ...
                    value={form.outlet_channel_id?.toString() ?? ""}
                    onChange={handleChange}
                    error={
                ...
                <div>
                  <InputFields
                    ...
                    label="Subcategory"
                    name="subcategory_id"
                    value={form.subcategory_id?.toString() ?? ""}
                    onChange={handleChange}
...
/>
  ...
  label="Payment Type"
  name="payment_type"
  value={form.payment_type?.toString() ?? ""}
  onChange={handleChange}
  error={touched.payment_type && errors.payment_type}
  options={[
    { value: "1", label: "Cash" },
    { value: "2", label: "Credit" },
    { value: "3", label: "TC - Bill to Bill" },
  ]}
/>
                ...
                <div>
                  <InputFields
                    ...
                    value={form.threshold_radius?.toString() ?? ""}
                    onChange={handleChange}
                    error={touched.threshold_radius && errors.threshold_radius}
                  />
                  {touched.threshold_radius && errors.threshold_radius && (
                    <div className="text-red-500 text-xs mt-1">
                      {errors.threshold_radius}
                    </div>
                  )}
                </div>
                <div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-semibold text-gray-900">
            {isEditMode ? "Edit Agent Customer" : "Add Agent Customer"}
          </h1>
        </div>
      </div>
      <StepperForm
